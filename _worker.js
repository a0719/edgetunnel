...
export default {
	async fetch(request, env, ctx) {
		let 请求URL文本 = request.url.替换(/%5[Cc]/g, '').替换(/\\/g, '');
		...
		if (!请求URL主体部分.includes('?') && /%3f/i.test(请求URL主体部分)) {
			...
			请求URL文本 = 请求URL主体部分.替换(/%3f/i, '?') + 请求URL锚点部分;
		}
		...
		const hosts = env.HOST ? (await 整理成数组(env.HOST)).map(h => h.toLowerCase().替换(/^https?:\/\//, '').split('/')[0].split(':')[0]) : [url.hostname];
		const host = hosts[0];
